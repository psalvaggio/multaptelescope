...
TwoCassegrain::TwoCassegrain(const mats::SimulationConfig& params,
    ...
    : Aperture(params, sim_index),
      ...
      opd_(),
      opd_est_() {
  // Copy over the Two Cassegrain-specific parameters.
  two_cassegrain_params_ =
      this->aperture_params().GetExtension(two_cassegrain_params);
  ...
  double subap_fill_factor = two_cassegrain_params_.subaperture_fill_factor();
  ...
  // Compute the angle of each of the arms.
  const double kInitialAngle = 0;
...
TwoCassegrain::~TwoCassegrain() {}

Mat TwoCassegrain::GetApertureTemplate() {
  ...
  if (max_val > 1) {
    mainLog() << "Error: Cassegrain subapertures are overlapping!" << endl;
  }
...
Mat TwoCassegrain::GetOpticalPathLengthDiff() {
...
Mat TwoCassegrain::GetOpticalPathLengthDiffEstimate() {
...
Mat TwoCassegrain::OpticalPathLengthDiffPtt(const vector<double>& ptt_vals) {
  const int kSize = params().array_size();
